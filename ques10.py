...
# -------------------------------
# Sweep configuration with three fixed configurations.
# -------------------------------
sweep_config = {
    'name': "mnist_configs",
    'method': 'grid',
...
def forward_propagation(x, weights, biases, activation_type='sigmoid'):
    ...
    for i in range(L):
        ...
        h_values.append(h)
    h_values = h_values[1:]  # Remove the input so that h_values[0] is output of first layer.
...
def backpropagation(a_values, h_values, weights, biases, x, y_true, activation_type='sigmoid'):
    ...
    # For cross entropy loss, derivative is: y_hat - y_true
...
# -------------------------------
# Optimizer update functions (unchanged)
# -------------------------------
...
def train_network(X, Y, weights, biases, epochs, learning_rate, batch_size=32,
                  activation_type='sigmoid', weight_decay=0.0, optimizer='sgd', val_split=0.1, loss_function="cross_entropy", **optimizer_kwargs):
    ...
    for epoch in range(epochs):
        ...
        for b in range(num_batches):
            ...
            for i in range(start, end):
                ...
                y_hat = h_vals[-1]
                # Only cross entropy loss is used here.
            ...
            weights, biases, optimizer_state = update_parameters(weights, biases, grad_w_avg_list, grad_b_avg_list, optimizer_state, optimizer, learning_rate)
...
# -------------------------------
# Run Experiment Function for the Fixed Configurations (Cross Entropy Only on MNIST)
# -------------------------------
def run_experiment():
    run = wandb.init()
    cfg = wandb.config.config  # Get the nested configuration dictionary

    run.name = f"Config: epochs {cfg['num_epochs']} hidden_layers {cfg['hiddenlayers']} hidden_size {cfg['hiddennodes']} learning_rate {cfg['learning_rate']} opt {cfg['optimizer']} batch_size {cfg['batch_size']} init {cfg['initializer']} activation {cfg['activation_func']} weight_decay {cfg['weight_decay']}"
    ...
    # Initialize parameters using the initializer from cfg
    weights, biases = initialize_parameters(layer_sizes, weight_init=cfg['initializer'].lower())

    # Train model using Cross Entropy Loss only
...
# -------------------------------
# Run W&B Sweep Agent for the Fixed Configurations (Cross Entropy Only on MNIST)
# -------------------------------
wandb.agent("airg681q", run_experiment, project="DA6401-Assignment1", count=3)
